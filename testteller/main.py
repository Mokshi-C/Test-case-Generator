...
@app.command()
def serve(
    host: Annotated[str, typer.Option("--host", help="Host for the HTTP API server.")] = "0.0.0.0",
    port: Annotated[int, typer.Option("--port", help="Port for the HTTP API server.")] = 8080,
    reload: Annotated[bool, typer.Option("--reload", help="Enable development auto-reload.")] = False,
):
    """Start the TestTeller HTTP API server."""
    import uvicorn

    uvicorn.run("testteller.api.app:app", host=host, port=port, reload=reload)
